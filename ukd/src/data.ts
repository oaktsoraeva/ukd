...
/**
 * Оператор ЭДО контрагента — строка в карточке «Контрагент» (узел 5040:12722).
 * В макете рядом с аватаром ещё и значок оператора, но логотипов операторов
 * нет ни в ките, ни в библиотеке логотипов
 */
export const EDO_OPERATOR = 'Диадок'
...
/**
 * Заглушка с названием: сообщение называет, во что попал клик, и
 * подсказывает, что в прототипе живёт только «Корректировка».
 */
export function notImplementedFor(label: string): string {
  return `«${label}» не реализован в прототипе. Здесь доступна только «Корректировка».`
}
